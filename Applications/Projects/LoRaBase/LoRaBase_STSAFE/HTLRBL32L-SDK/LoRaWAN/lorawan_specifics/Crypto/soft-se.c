        ...
        if( SeNvmCtx.KeyList[i].KeyID == keyID )
        {
            *keyItem = &( SeNvmCtx.KeyList[i] );
            ...
            for(int x=0;x<KEY_SIZE;x++){
            	printf("%02x ",SeNvmCtx.KeyList[i].KeyValue[x]);
            }
            printf("\n");
...
static SecureElementStatus_t ComputeCmac( uint8_t *micBxBuffer, uint8_t *buffer, uint16_t size, KeyIdentifier_t keyID, uint32_t* cmac )
{
	...
	return(ht_compute_cmac( micBxBuffer, buffer, size, keyID, cmac,OVER_THE_AIR_ACTIVATION));
...
SecureElementStatus_t SecureElementSetKey( KeyIdentifier_t keyID, uint8_t* key )
{
	...
	return ht_key_set(keyID, key,OVER_THE_AIR_ACTIVATION);
...
{
#ifdef HT_CRYPTO
	return (ht_aes_encrypt(buffer, size, keyID, encBuffer,OVER_THE_AIR_ACTIVATION ));
