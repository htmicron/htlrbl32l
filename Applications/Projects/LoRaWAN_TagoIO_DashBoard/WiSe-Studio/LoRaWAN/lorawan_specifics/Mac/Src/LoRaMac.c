...
static void OnRadioRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr)
{
    RxBuffer.LastRxDone = TimerGetCurrentTime();
    RxBuffer.Payload = payload;
    RxBuffer.Size = size;
    RxBuffer.Rssi = rssi;
    RxBuffer.Snr = snr;
...
#ifdef PUSH_BUTTON_APP
	if(HT_PB_GetLoraProcess() == PROCESS_LORA_RX_WINDOW_2)
...
static void ProcessRadioRxDone(void)
{
    ...
    uint8_t *payload = RxBuffer.Payload;
    uint16_t size = RxBuffer.Size;
    int16_t rssi = RxBuffer.Rssi;
    int8_t snr = RxBuffer.Snr;

#ifdef PUSH_BUTTON_APP
    HT_PB_SetState(SM_RX_LORA);
#endif
...
static void LoRaMacHandleIrqEvents(void)
{
    ...
    if (events.Value != 0)
    {
        ...
        //printf("radio was reset %u times\n",n);
    }
}
...
static void ProcessMacCommands(uint8_t *payload, uint8_t macIndex, uint8_t commandsSize, int8_t snr, LoRaMacRxSlot_t rxSlot)
{
    ...
    while (macIndex < commandsSize)
    {
        ...
        case SRV_MAC_BEACON_TIMING_ANS:
        {
            ...
            LoRaMacClassBBeaconTimingAns(beaconTimingDelay, beaconTimingChannel, RxBuffer.LastRxDone);
            break;
        }
