    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            for( uint8_t i = 0; i < US915_MAX_NB_CHANNELS - 8; i++ )
            {
								...
								//NvmCtx.Channels[i].Frequency = 915200000 + i * 200000;//AU915 
            ...
            for( uint8_t i = US915_MAX_NB_CHANNELS - 8; i < US915_MAX_NB_CHANNELS; i++ )
            {
                NvmCtx.Channels[i].Frequency = 903000000 + ( i - ( US915_MAX_NB_CHANNELS - 8 ) ) * 1600000; 
							//	NvmCtx.Channels[i].Frequency = 915200000 + ( i - ( US915_MAX_NB_CHANNELS - 8 ) ) * 1600000; //AU915
                NvmCtx.Channels[i].DrRange.Value = ( DR_4 << 4 ) | DR_4;
                NvmCtx.Channels[i].Band = 0;
            }

            // ChannelsMask
//						NvmCtx.ChannelsDefaultMask[0] = 0xFF00; //923.3 RAK2245
...
//            NvmCtx.ChannelsDefaultMask[0] = 0xFFFF; //ORIGINAL
...
#endif
#ifdef RX_SPREADING_FACTOR
	phyDr = RX_SPREADING_FACTOR;
#endif	
#ifdef RX_CODERATE
	coderate = RX_CODERATE;
#endif	
#ifdef RX_BANDWIDTH
		rxConfig->Bandwidth = RX_BANDWIDTH;
#endif

		Radio.SetChannel( frequency );
		
    Radio.SetRxConfig( MODEM_LORA, rxConfig->Bandwidth, phyDr, coderate, 0, 8, 0, false, 0, true, 0, 0, true, rxConfig->RxContinuous );
...
bool RegionUS915TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
...
#ifdef TX_FREQUENCY
		NvmCtx.Channels[txConfig->Channel].Frequency = TX_FREQUENCY;
#endif
	
#ifdef TX_SPREADING_FACTOR
	phyDr = TX_SPREADING_FACTOR;
#endif	
#ifdef TX_CODERATE
	coderate = TX_CODERATE;
#endif	
    ...
    Radio.SetTxConfig( MODEM_LORA, phyTxPower, 0, bandwidth, phyDr, coderate, 8, false, true, 0, 0, false, 4000 );
