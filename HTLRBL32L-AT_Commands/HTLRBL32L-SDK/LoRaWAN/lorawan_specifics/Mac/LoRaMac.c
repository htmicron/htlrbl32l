...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
	...
	at_control.rx_stats.rssi=rssi;
	at_control.rx_stats.snr=snr;
...
static void LoRaMacHandleIrqEvents( void )
{
    ...
    if( events.Value != 0 )
    {
        ...
        {
			uart_locked = 0;
			report_lora_execution_error(LORA_ERROR_TX_TIMEOUT);
...
bool LoRaMacIsBusy( void )
{
	...
	report_lora_execution_error(LORA_ERROR_BUSY);
	notify();
    return true;
}
...
//		
//		
//		printf("ProcessMacCommands -- macIndex :%d, commandsSize:%d\n",macIndex,commandsSize);
    while( macIndex < commandsSize )
    {
        // Decode Frame MAC commands
				//printf("payload[macIndex++]: %d\n",payload[macIndex++]);
...
//                MacCtx.NvmCtx->MacParams.ReceiveDelay1 = (delay * 1000)-300;
//                MacCtx.NvmCtx->MacParams.ReceiveDelay2 = MacCtx.NvmCtx->MacParams.ReceiveDelay1 + 1000; 
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t* primitives, LoRaMacCallback_t* callbacks, LoRaMacRegion_t region )
{
    ...
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.ChannelsTxPower = at_control.tx_power;
    ...
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.MaxRxWindow = at_control.rx_window_duration;

    getPhy.Attribute = PHY_RECEIVE_DELAY1;
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.ReceiveDelay1 = at_control.delay_rx1;

    getPhy.Attribute = PHY_RECEIVE_DELAY2;
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.ReceiveDelay2 = at_control.delay_rx2;

    getPhy.Attribute = PHY_JOIN_ACCEPT_DELAY1;
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.JoinAcceptDelay1 = at_control.join_delay_rx1;

    getPhy.Attribute = PHY_JOIN_ACCEPT_DELAY2;
    phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
    MacCtx.NvmCtx->MacParamsDefaults.JoinAcceptDelay2 = at_control.join_delay_rx2;
...
LoRaMacStatus_t LoRaMacQueryTxPossible( uint8_t size, LoRaMacTxInfo_t* txInfo )
{
    ...
    if( ( LORA_MAC_COMMAND_MAX_FOPTS_LENGTH >= macCmdsSize ) && ( txInfo->CurrentPossiblePayloadSize >= macCmdsSize ) )
    {
        ...
        {
			report_lora_execution_error(LORA_ERROR_INVALID_FOPTS_PAYLOAD_SIZE);
           return LORAMAC_STATUS_LENGTH_ERROR;
        }
    }
    else
