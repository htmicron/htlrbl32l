...
static void OnTxTimerEvent(void *context)
{
			...
			//Takes reading from internal temperature sensor
			sensor_ADC(&sensor_adc2);
			appData.Buff = 0;

			//Shifts first two decimal places then passes values to struct
			payload_s.temperature = sensor_adc2.temp*100;
			payload_s.humi = 0*100;
			payload_s.p = 0*100;
			//Casting struct as a byte vector in payload buffer
			appData.Buff = (uint8_t*)&payload_s;


			//Prints temperature reading and payload as hex
			/*
			printf("\n===============\nTEMP: %d\nPAYLOAD: \"", payload_s.temperature);
			for(i = 0; i < sizeof(HT_payload); i++)
			{
			...
			*/
			...
			//Buffer size is the size of the payload struct
...
{
  if (!status && certif_running)
  { //if test mode changes to false enable TX regular cycle
    HAL_Delay(100);
